...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout {
	...
	public final void setMode(Mode mode) {
		if (mode != mMode) {
			if (DEBUG) {
				LogPrinter.d(LOG_TAG, "Setting mode to: " + mode);
			}
			mMode = mode;
			updateUIForMode();
		}
	...
	public void setPullLabel(String pullLabel) {
		setPullLabel(pullLabel, pullLabel, Mode.BOTH);
	}
	...
	public void setPullLabel(String pullDownLabel, String pullUpLabel, Mode mode) {
		if (null != mHeaderLayout && mode.canPullDown()) {
			mHeaderLayout.setPullLabel(pullDownLabel);
		}
		if (null != mFooterLayout && mode.canPullUp()) {
			mFooterLayout.setPullLabel(pullUpLabel);
		}
	}
	 ...
	 * method and return an instance of your custom class. Be sure to set the ID
	 * of the view in this method, especially if you're using a ListActivity or
	 * ListFragment.
	 * 
	 * @param context
